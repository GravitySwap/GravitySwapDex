...
import { GRAVITY } from 'gravityswap_core-sdk' //'@sushiswap/core-sdk'
...
const MeowshiButton: FC<MeowshiButtonProps> = ({ meowshiState }) => {
  ...
  // @ts-ignore TYPE NEEDS FIXING
  const sushiBalance = useTokenBalance(account, GRAVITY[ChainId.SEPOLIA])
  // const sushiBalance = useTokenBalance(account, SUSHI[ChainId.ETHEREUM])
  // @ts-ignore TYPE NEEDS FIXING
  const xSushiBalance = useTokenBalance(account, XGRAVITY)
  ...
  const { approvalState, approve, meow, unmeow, meowSushi, unmeowSushi } = useMeowshi(
    currencies[Field.INPUT] === GRAVITY[ChainId.SEPOLIA]
  ...
  const handleSubmit = async () => {
    ...
    let tx
    if (doMeow) {
      if (currencies[Field.INPUT]?.symbol === 'GVT') {
        //'SUSHI'
        tx = await meowSushi({
          // @ts-ignore TYPE NEEDS FIXING
      ...
      if (currencies[Field.INPUT]?.symbol === 'xGVT') {
        //'xSUSHI'
        tx = await meow({
          // @ts-ignore TYPE NEEDS FIXING
          value: parseUnits(fields[Field.INPUT], sushiBalance.currency.decimals),
      ...
      if (currencies[Field.OUTPUT]?.symbol === 'GVT') {
        //'SUSHI'
      ...
      if (currencies[Field.OUTPUT]?.symbol === 'xGVT') {
        //'xGVT
        tx = await unmeow({
          // @ts-ignore TYPE NEEDS FIXING
          value: parseUnits(fields[Field.INPUT], sushiBalance.currency.decimals),
