...
import { ChainId, getCustomChainIds } from 'app/config/rpc'
import { useCurrency } from 'app/hooks/Tokens'
import { useActiveWeb3React } from 'app/services/web3'
import { Currency, GRAVITY, NATIVE } from 'gravityswap_core-sdk' //'@sushiswap/core-sdk'
import { useRouter } from 'next/router'
import { useCallback, useMemo } from 'react'

const customChainIds = getCustomChainIds()
...
const useCurrenciesFromURL = (): {
  ...
  const isCustomeChain = chainId && customChainIds.includes(chainId)
  const currencyA =
    useCurrency(getToken(router.query.tokens?.[0], chainId)) || (chainId && NATIVE[chainId]) || undefined
  const currencyB =
    useCurrency(getToken(router.query.tokens?.[1], chainId)) ||
    (chainId && !isCustomeChain && GRAVITY[chainId as ChainId as keyof typeof GRAVITY]) ||
    // (chainId && !isCustomeChain && SUSHI[chainId as ChainId as keyof typeof SUSHI]) ||
    undefined
