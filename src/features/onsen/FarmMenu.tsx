...
const OnsenFilter = ({ account, chainId }: { account?: string | null; chainId?: number }) => {
  ...
  const items = useMemo(() => {
    const map: Record<string, ReactNode> = {
      ...
      [FarmFilter.Gravity]:
        chainId && [ChainId.SEPOLIA, ChainId.ETHEREUM].includes(chainId) ? (
          <MenuLink href={'/farm?filter=gravity'} label={i18n._(t`GravitySwap Farms`)} />
        ) : undefined,
    }
