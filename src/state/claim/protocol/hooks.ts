...
export function useUserUnclaimedAmount(account: string | null | undefined): CurrencyAmount<Currency> | undefined {
  ...
  const isCustomChain = chainId && getCustomChainIds().includes(chainId)

  const sushi = chainId && !isCustomChain ? SUSHI[chainId as ChainId as keyof typeof SUSHI] : undefined
