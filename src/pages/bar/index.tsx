...
import { XGRAVITY } from 'app/config/tokens/sepolia'
...
import { BAR_ADDRESS, SUSHI, ZERO } from 'gravityswap_core-sdk' //'@sushiswap/core-sdk'
import Image from 'next/image'
import Link from 'next/link'
import { NextSeo } from 'next-seo'
import React, { useState } from 'react'

import TokenImg from '../../../public/images/GravitySwapToken.png'
...
export default function Stake() {
  ...
  const gravityBalance = useTokenBalance(account ?? undefined, SUSHI[ChainId.ETHEREUM])
  const xGravityBalance = useTokenBalance(account ?? undefined, XGRAVITY)
  ...
  const balance = activeTab === 0 ? gravityBalance : xGravityBalance
  ...
  const { data: block1m } = useOneMonthBlock({ chainId: ChainId.SEPOLIA })

  const { data: block3m } = useThreeMonthBlock({ chainId: ChainId.SEPOLIA })

  const { data: block6m } = useSixMonthBlock({ chainId: ChainId.SEPOLIA })

  const { data: block1y } = useOneYearBlock({ chainId: ChainId.SEPOLIA })

  const { data: ethPrice } = useNativePrice({ chainId: ChainId.SEPOLIA })

  const xGravity = useTokens({
    chainId: ChainId.SEPOLIA,
    variables: { where: { id: XGRAVITY.address.toLowerCase() } },
  })?.[0]
  ...
  const [xGravityPrice] = [xGravity?.derivedETH * ethPrice, xGravity?.derivedETH * ethPrice * bar?.totalSupply]

  const apy1m = (bar?.ratio / bar1m?.ratio - 1) * 12 * 100

  return (
    <div className="banner bar-page">
      <Container id="bar-page" className="py-4 mx-auto md:py-8 lg:py-12" maxWidth="full">
        <NextSeo
          title={`Stake Gravity`}
          description={`Stake Gravity in return for xGravity, an interest bearing and fungible ERC20 token designed to share revenue generated by all Gravity products.`}
          // openGraph={{
        ...
        <div className="flex flex-col w-full min-h-full">
          <div className="flex justify-center mb-6">
            <div className="w-full mt-auto mb-2 max-w-7xl">
              <div className="heading-text">
                <h1>{i18n._(t`Maximize yield by staking Gravity for xGravity`)}</h1>
                {/* <p>
                  {' '}
                  {i18n._(
                ...
                <div className="w-full max-w-xl px-3 pt-2 pb-6 rounded bg-dark-900 md:pb-9 md:pt-4 md:px-8">
                  ...
                  <div className="flex items-center justify-between w-full mt-6">
                    <p className="font-bold text-large md:text-2xl text-high-emphesis">
                      {activeTab === 0 ? i18n._(t`Stake GRAVITY`) : i18n._(t`Unstake`)}
                    </p>
                    <div className="border-gradient-r-pink-red-light-brown-dark-pink-red border-transparent border-solid border rounded-3xl px-4 md:px-3.5 py-1.5 md:py-0.5 text-high-emphesis text-xs font-medium md:text-base md:font-normal">
                      {`1 xGravity = ${Number(bar?.ratio ?? 0)?.toFixed(4)} Gravity`}
                    </div>
                  </div>
                  ...
                  <div className="relative w-full h-0 pointer-events-none bottom-14">
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        ...
                        >
                          {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}GRAVITY`}
                        </p>
                      </div>
                ...
                <div className="flex flex-wrap">
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex flex-col justify-center">
                        <p className="text-sm font-bold md:text-lg text-high-emphesis">
                          {xGravityBalance ? xGravityBalance.toSignificant(4) : '-'}
                        </p>
                        <p className="text-sm md:text-base text-primary">XGRAVITY</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col flex-grow">
                    ...
                    <div className="flex items-center ml-8 space-x-4 md:ml-0">
                      ...
                      <div className="flex flex-col justify-center">
                        <p className="text-sm font-bold md:text-lg text-high-emphesis">
                          {gravityBalance ? gravityBalance.toSignificant(4) : '-'}
                        </p>
                        <p className="text-sm md:text-base text-primary">GRAVITY</p>
                      </div>
                    </div>
                  </div>
